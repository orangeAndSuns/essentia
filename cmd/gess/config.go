...
type essstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gessConfig struct {
	Ess       ess.Config
	Shh       whisper.Config
	Node      node.Config
	Essstats  essstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *gessConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gess.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gessConfig) {
	// Load defaults.
	cfg := gessConfig{
	...
	if ctx.GlobalIsSet(utils.EthStatsURLFlag.Name) {
		cfg.Essstats.URL = ctx.GlobalString(utils.EthStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if cfg.Essstats.URL != "" {
		utils.RegisterEthStatsService(stack, cfg.Essstats.URL)
	}
	return stack
}
