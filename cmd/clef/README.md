Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for gess's account management.
This allows DApps not to depend on gess's account management. When a DApp wants to sign data it can send the data to
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.essentia/keystore")
...
The general flow for signing a transaction using e.g. gess is as follows:
![image](sign_flow.png)

In this case, `gess` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `ess.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gess should be started in `gess --external_signer localhost:8550`.
    - Currently, the Gess APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gess api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus gess/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gess. The messages are
  ...
  "params": [
    {
      "accounts": [
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.essentia/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.essentia/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
  ...
  "params": [
    {
      "address": "0x123409812340981234098123409812deadbeef42",
      "raw_data": "0x01020304",
      "message": "\u0019Essentia Signed Message:\n4\u0001\u0002\u0003\u0004",
...
* A Graphical UI MUST show the blocky-identicon for essentia addresses.
