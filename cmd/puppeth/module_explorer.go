...
ADD essstats.json /essstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ess-net-intelligence-api && pm2 start /essstats.json)' >  explorer.sh && \
...
// explorerEssstats is the configuration file for the essstats javascript client.
var explorerEssstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.essentia
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Essstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	essstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEssstats)).Execute(essstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.essstats[:strings.Index(config.essstats, ":")],
		"Secret": config.essstats[strings.Index(config.essstats, ":")+1 : strings.Index(config.essstats, "@")],
		"Host":   config.essstats[strings.Index(config.essstats, "@")+1:],
	})
	files[filepath.Join(workdir, "essstats.json")] = essstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.essentia"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		essstats: infos.envvars["STATS"],
	}
	return stats, nil
}
