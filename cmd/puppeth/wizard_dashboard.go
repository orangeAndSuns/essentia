...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"essstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "essstats":
				if infos, err := checkEssstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have essstats running, ask whether to make the secret public or not
	if w.conf.essstats != "" {
		fmt.Println()
		fmt.Println("Include essstats secret on dashboard (y/n)? (default = yes)")
