...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gess --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gess --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --essstats \'{{.Essstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
