...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.essstats == "" {
		log.Error("No essstats server configured")
		return
	}
	...
	if infos.essstats == "" {
		...
		infos.essstats = w.readString() + ":" + w.conf.essstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.essstats)
		infos.essstats = w.readDefaultString(infos.essstats) + ":" + w.conf.essstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.ESShash != nil {
			// ESShash  based miners only need an essbase to mine against
			fmt.Println()
			if infos.essbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.essbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.essbase)
				infos.essbase = w.readDefaultAddress(common.HexToAddress(infos.essbase)).Hex()
			}
