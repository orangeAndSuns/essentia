...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for essstats availability")
	if infos, err := checkEssstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["essstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["essstats"] = infos.Report()
		essstats = infos.config
	}
