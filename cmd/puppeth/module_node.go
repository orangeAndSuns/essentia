...
var nodeDockerfile = `
FROM essentia/client-go:latest
...
{{end}}
RUN \
  echo 'gess --cache 512 init /genesis.json' > gess.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.essentia/keystore/ && cp /signer.json /root/.essentia/keystore/' >> gess.sh && \{{end}}
	echo $'gess --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --essstats \'{{.Essstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .ESSBase}}--essbase {{.ESSBase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gess.sh

ENTRYPOINT ["/bin/sh", "gess.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.essentia{{if .Esshashdir}}
      - {{.Esshashdir}}:/root/.esshash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.essbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Esshashdir": config.esshashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Essstats":   config.essstats[:strings.Index(config.essstats, ":")],
		"ESSBase":    config.essbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Essstats username":        info.essstats,
	}
	if info.gasTarget > 0 {
		...
		if info.essbase != "" {
			// ESShash  proof-of-work miner
			report["ESShash directory"] = info.esshashdir
			report["Miner account"] = info.essbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gess --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.essentia"],
		esshashdir: infos.volumes["/root/.esshash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		essstats:   infos.envvars["STATS_NAME"],
		essbase:    infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
