...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "esskey-test")
	if err != nil {
	...
	// Create the key.
	generate := runESSKey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runESSKey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runESSKey(t, "verifymessage", address, signature, message)
