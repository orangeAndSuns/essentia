...
var (
	...
	essPortFlag = flag.Int("ethport", 30303, "Listener port for the devp2p connection")
	...
	statsFlag   = flag.String("essstats", "", "Essstats network monitoring auth string")
...
func main() {
	...
	faucet, err := newFaucet(genesis, *essPortFlag, essnodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type faucet struct {
	...
	client *essclient.Client   // Client connection to the Essentia chain
...
func newFaucet(genesis *core.Genesis, port int, essnodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the essstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEssentia
			ctx.Service(&serv)
			return essstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := essclient.NewClient(api)
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/orangeAndSuns/essentia/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
