...
import (
	...
	"github.com/orangeAndSuns/essentia/cmd/utils"
	"github.com/orangeAndSuns/essentia/common"
	"github.com/orangeAndSuns/essentia/console"
	...
	"github.com/orangeAndSuns/essentia/swarm"
	bzzapi "github.com/orangeAndSuns/essentia/swarm/api"
	swarmmetrics "github.com/orangeAndSuns/essentia/swarm/metrics"
	"github.com/orangeAndSuns/essentia/swarm/tracing"
...
var (
	...
	SwarmStorePath = cli.StringFlag{
		Name:   "store.path",
		Usage:  "Path to leveldb chunk DB (default <$GESS_ENV_DIR>/swarm/bzz-<$BZZ_KEY>/chunks)",
...
// This init function sets defaults so cmd/swarm can run alongside gess.
func init() {
...
func init() {
	...
	app.Copyright = "Copyright 2013-2016 The qwerty123 Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.essentia/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.essentia/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:             dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.essentia/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.essentia/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:             dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gess only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gess and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the essentia node
	...
	//register BZZ as node.Service in the essentia node
...
func registerBzzService(bzzconfig *bzzapi.Config, stack *node.Node) {
	...
	//register within the essentia node
