...
func newTester(t *testing.T, confOverride func(*ess.Config)) *tester {
	...
	ethConf := &ess.Config{
		...
		ESSBase: common.HexToAddress(testAddress),
		ESShash: esshash.Config{
			PowMode: esshash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(ethConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ess.New(ctx, essConf) }); err != nil {
