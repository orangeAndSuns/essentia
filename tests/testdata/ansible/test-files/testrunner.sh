...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-ethereum/build/test/createRandomStateTest\" essentia/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyethereum
	OUTPUT_PYTHON="$(docker run --rm essentia/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm essentia/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm essentia/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
