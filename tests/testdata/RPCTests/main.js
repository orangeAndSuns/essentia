...
//requires essentia ess path on input

var async = require("async");
var utils = require('./modules/utils.js');
var testutils = require('./modules/testutils.js');
var essconsole = require('./modules/ethconsole.js');

var esspath = process.argv[2];
...
{
	utils.cLog("Executable '" + esspath + "' not found!");
	utils.cLog("Please, set ess path. Usage: node main.js <ethpath>");
	return;
}
...
function prepareDynamicVars(finished)
{
  async.series([
	...
	function(cb) {
		dynamic["account"] = essconsole.getLastResponse();
		utils.mkdir(testdir);
	...
	function(cb) {
		dynamic["node1_ID"] = essconsole.getLastResponse().id;
		cb();
	}
  ], function() { finished(); })
}
...
function runAllTests(finished)
{
	...
	updateDynamic = function updateDynamic()
	{
		async.series([
			...
			function(cb) {
				dynamic["node1_lastblock"] = essconsole.getLastResponse();
				cb();
			},
			function(cb) {
			...
			function(cb) {
				dynamic["node2_lastblock"] = essconsole.getLastResponse();
				cb();
			}
	    	], function() { nextTest(); });
	}
