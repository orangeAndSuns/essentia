...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout ess_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for EssSubscribe requests
}
...
// EssSubscribe registers a subscripion under the "ess" namespace.
func (c *Client) EssSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. EssSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through EssSubscribe.
