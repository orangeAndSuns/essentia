...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("ess", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
// In this test, the connection drops while EssSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("ess", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("qwerty123-test-ipc-%d-%d", os.Getpid(), rand.Int63())
