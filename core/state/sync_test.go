...
func makeTestState() (Database, common.Hash, []*testAccount) {
	...
	db := NewDatabase(essdb.NewMemDatabase())
...
// account array.
func checkStateAccounts(t *testing.T, db essdb.Database, root common.Hash, accounts []*testAccount) {
...
func checkTrieConsistency(db essdb.Database, root common.Hash) error {
...
func checkStateConsistency(db essdb.Database, root common.Hash) error {
...
func TestEmptyStateSync(t *testing.T) {
	...
	if req := NewStateSync(empty, essdb.NewMemDatabase()).Missing(1); len(req) != 0 {
...
func TestIncompleteStateSync(t *testing.T) {
	...
	checkTrieConsistency(srcDb.TrieDB().DiskDB().(essdb.Database), srcRoot)
