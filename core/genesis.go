...
func SetupGenesisBlock(db essdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllEsshashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllEsshashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db essdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db essdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
	...
	config := g.Config
	if config == nil {
		config = params.AllEsshashProtocolChanges
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db essdb.Database) *types.Block {
...
func GenesisBlockForTesting(db essdb.Database, addr common.Address, balance *big.Int) *types.Block {
...
// DeveloperGenesisBlock returns the 'gess --dev' genesis block. Note, this must
// be seeded with the
