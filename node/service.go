...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (essdb.Database, error) {
	if ctx.config.DataDir == "" {
		return essdb.NewMemDatabase(), nil
	}
	db, err := essdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
