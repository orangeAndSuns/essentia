...
func (n *Node) OpenDatabase(name string, cache, handles int) (essdb.Database, error) {
	if n.config.DataDir == "" {
		return essdb.NewMemDatabase(), nil
	}
	return essdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
