...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gess is "gess". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gess", keep that.
	if name == "gess" || name == "gess-testnet" {
		name = "Gess"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gess" instances.
var isOldGessResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gess 1.4 are used if they exist.
	if c.name() == "gess" && isOldGessResource[path] {
		oldpath := ""
		if c.Name == "gess" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "qwerty123-keystore")
		ephemeral = keydir
	}
