...
import (
	...
	"github.com/orangeAndSuns/essentia/p2p/simulations/adapters"
)
...
type Network struct {
	...
	nodeMap map[discover.ESSNodeID]int
...
func NewNetwork(nodeAdapter adapters.NodeAdapter, conf *NetworkConfig) *Network {
	return &Network{
		...
		nodeMap:       make(map[discover.ESSNodeID]int),
...
func (net *Network) NewNodeWithConfig(conf *adapters.NodeConfig) (*Node, error) {
	...
	if conf.Reachable == nil {
		conf.Reachable = func(otherID discover.ESSNodeID) bool {
...
func (net *Network) Start(id discover.ESSNodeID) error {
	return net.startWithSnapshots(id, nil)
}

// startWithSnapshots starts the node with the given ID using the give
// snapshots
func (net *Network) startWithSnapshots(id discover.ESSNodeID, snapshots map[string][]byte) error {
	net.lock.Lock()
	defer net.lock.Unlock()
	node := net.getNode(id)
	if node == nil {
...
func (net *Network) watchPeerEvents(id discover.ESSNodeID, events chan *p2p.PeerEvent, sub event.Subscription) {
	defer func() {
		sub.Unsubscribe()
...
func (net *Network) Stop(id discover.ESSNodeID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
	node := net.getNode(id)
	if node == nil {
...
func (net *Network) Connect(oneID, otherID discover.ESSNodeID) error {
...
func (net *Network) Disconnect(oneID, otherID discover.ESSNodeID) error {
...
func (net *Network) DidConnect(one, other discover.ESSNodeID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
// "other" node
func (net *Network) DidDisconnect(one, other discover.ESSNodeID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) DidSend(sender, receiver discover.ESSNodeID, proto string, code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
		Protocol: proto,
...
func (net *Network) DidReceive(sender, receiver discover.ESSNodeID, proto string, code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
		Protocol: proto,
...
// exist
func (net *Network) GetNode(id discover.ESSNodeID) *Node {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) GetConn(oneID, otherID discover.ESSNodeID) *Conn {
	net.lock.Lock()
	defer net.lock.Unlock()
...
// already exist
func (net *Network) GetOrCreateConn(oneID, otherID discover.ESSNodeID) (*Conn, error) {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) InitConn(oneID, otherID discover.ESSNodeID) (*Conn, error) {
	net.lock.Lock()
	defer net.lock.Unlock()
	if oneID == otherID {
...
func (net *Network) Reset() {
	...
	net.nodeMap = make(map[discover.ESSNodeID]int)
...
func (n *Node) ID() discover.ESSNodeID {
	return n.Config.ID
}
...
// nodes
func ConnLabel(source, target discover.ESSNodeID) string {
	var first, second discover.ESSNodeID
