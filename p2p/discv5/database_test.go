...
func TestNodeDBInt64(t *testing.T) {
	db, _ := newNodeDB("", Version, ESSNodeID{})
	defer db.close()
...
func TestNodeDBPersistency(t *testing.T) {
	...
	db, err := newNodeDB(filepath.Join(root, "database"), Version, ESSNodeID{})
	if err != nil {
	...
	db, err = newNodeDB(filepath.Join(root, "database"), Version+1, ESSNodeID{})
	if err != nil {
		t.Fatalf("failed to open persistent database: %v", err)
	}
