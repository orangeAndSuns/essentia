...
import (
	...
	"github.com/orangeAndSuns/essentia/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gess console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ess.getBlock(ess.getTransaction(tx).blockHash);
  var genesis = ess.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ess.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ess.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(essdb.NewMemDatabase(), test.Genesis.Alloc)
