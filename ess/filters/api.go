...
// information related to the Essentia protocol such als blocks, transactions and logs.
...
// It is part of the filter package since polling goes with ess_getFilterChanges.
//
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(essentia.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as essentia.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria essentia.FilterQuery
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(essentia.FilterQuery(crit), logs)
	if err != nil {
		return rpc.ID(""), err
	}
