...
// PublicEssentiaAPI provides an API to access Essentia full node-related
// information.
type PublicEssentiaAPI struct {
	e *Essentia
}

// NewPublicEssentiaAPI creates a new Essentia protocol API for full nodes.
func NewPublicEssentiaAPI(e *Essentia) *PublicEssentiaAPI {
	return &PublicEssentiaAPI{e}
}

// ESSBase is the address that mining rewards will be send to
func (api *PublicEssentiaAPI) ESSBase() (common.Address, error) {
	return api.e.ESSBase()
}

// Coinbase is the address that mining rewards will be send to (alias for ESSBase)
func (api *PublicEssentiaAPI) Coinbase() (common.Address, error) {
	return api.ESSBase()
}

// Hashrate returns the POW hashrate
func (api *PublicEssentiaAPI) Hashrate() hexutil.Uint64 {
...
// SetESSBase sets the essbase of the miner
func (api *PrivateMinerAPI) SetESSBase(essbase common.Address) bool {
	api.e.SetESSBase(essbase)
	return true
}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	...
	for i, block := range blocks {
		...
		if results[i].Block, err = essapi.RPCMarshalBlock(block, true, true); err != nil {
