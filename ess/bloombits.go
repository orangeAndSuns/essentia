...
type BloomIndexer struct {
	...
	db  essdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db essdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := essdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
