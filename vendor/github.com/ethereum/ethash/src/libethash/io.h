...
// Reference: https://github.com/ethereum/wiki/wiki/ESShash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see esshash_io_prepare
enum esshash_io_rc {
...
/**
 * Logs a critical error in important parts of esshash. Should mostly help
...
/**
 * Prepares io for esshash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the esshash
 ...
 *                           https://github.com/ethereum/wiki/wiki/ESShash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum esshash_io_rc
 */
enum esshash_io_rc esshash_io_prepare(
	char const* dirname,
	esshash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* esshash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* esshash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool esshash_mkdir(char const* dirname);
 ...
 */
bool esshash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* esshash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereum/wiki/wiki/ESShash-DAG
 *
 ...
 */
bool esshash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool esshash_io_mutable_name(
	uint32_t revision,
