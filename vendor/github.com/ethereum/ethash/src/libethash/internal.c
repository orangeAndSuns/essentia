/*
  ...
  along with cpp-essentia.	If not, see <http://www.gnu.org/licenses/>.
*/
/** @file internal.c
...
// SeqMemoHash(s, R, N)
bool static esshash_compute_cache_nodes(
	node* const nodes,
	uint64_t cache_size,
	esshash_h256_t const* seed
)
{
...
)
{
	...
	for (uint32_t n = 0; n != max_n; ++n) {
		...
		esshash_calculate_dag_item(&(full_nodes[n]), n, light);
	}
	return true;
}

static bool esshash_hash(
	esshash_return_value_t* ret,
	...
	for (unsigned i = 0; i != ETHASH_ACCESSES; ++i) {
		...
		for (unsigned n = 0; n != MIX_NODES; ++n) {
			...
			if (full_nodes) {
				...
				node tmp_node;
				esshash_calculate_dag_item(&tmp_node, index * MIX_NODES + n, light);
				dag_node = &tmp_node;
			}
...
)
{
	...
	esshash_h256_t return_hash;
	esshash_quick_hash(&return_hash, header_hash, nonce, mix_hash);
	return esshash_check_difficulty(&return_hash, boundary);
}

esshash_light_t esshash_light_new_internal(uint64_t cache_size, esshash_h256_t const* seed)
{
	struct esshash_light *ret;
	...
	if (!esshash_compute_cache_nodes(nodes, cache_size, seed)) {
...
{
	esshash_h256_t seedhash = esshash_get_seedhash(block_number);
	esshash_light_t ret;
	ret = esshash_light_new_internal(esshash_get_cachesize(block_number), &seedhash);
...
)
{
  	esshash_return_value_t ret;
	ret.success = true;
	if (!esshash_hash(&ret, NULL, light, full_size, header_hash, nonce)) {
		ret.success = false;
	}
	return ret;
}
...
{
	uint64_t full_size = esshash_get_datasize(light->block_number);
	return esshash_light_compute_internal(light, full_size, header_hash, nonce);
}

static bool esshash_mmap(struct esshash_full* ret, FILE* f)
{
	int fd;
	char* mmapped_data;
	errno = 0;
	ret->file = f;
	if ((fd = esshash_fileno(ret->file)) == -1) {
		return false;
	}
	mmapped_data= mmap(
		NULL,
...
)
{
	...
	switch (esshash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, false)) {
	case ETHASH_IO_FAIL:
		// esshash_io_prepare will do all ETHASH_CRITICAL() logging in fail case
		goto fail_free_full;
	case ETHASH_IO_MEMO_MATCH:
		if (!esshash_mmap(ret, f)) {
	...
	case ETHASH_IO_MEMO_SIZE_MISMATCH:
		...
		if (esshash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) != ETHASH_IO_MEMO_MISMATCH) {
	...
	if (!esshash_compute_full_data(ret->data, full_size, light, callback)) {
...
{
	char strbuf[256];
	if (!esshash_get_default_dirname(strbuf, 256)) {
		return NULL;
	}
	uint64_t full_size = esshash_get_datasize(light->block_number);
	esshash_h256_t seedhash = esshash_get_seedhash(light->block_number);
	return esshash_full_new_internal(strbuf, seedhash, full_size, light, callback);
}
