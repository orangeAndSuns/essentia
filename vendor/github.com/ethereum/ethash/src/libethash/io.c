...
)
{
	...
	enum esshash_io_rc ret = ETHASH_IO_FAIL;
	// reset errno before io calls
	errno = 0;

	// assert directory exists
	if (!esshash_mkdir(dirname)) {
		ETHASH_CRITICAL("Could not create the esshash directory");
		goto end;
	}

	esshash_io_mutable_name(ETHASH_REVISION, &seedhash, mutable_name);
	char* tmpfile = esshash_io_create_filename(dirname, mutable_name, strlen(mutable_name));
	if (!tmpfile) {
	...
	if (!force_create) {
		...
		f = esshash_fopen(tmpfile, "rb+");
		if (f) {
			size_t found_size;
			if (!esshash_file_size(f, &found_size)) {
				fclose(f);
