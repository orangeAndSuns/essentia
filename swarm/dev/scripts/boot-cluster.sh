...
#
# The cluster contains a bootnode, a gess node and multiple swarm nodes, with
...
# static gess configuration
GESS_IP="192.168.33.3"
GESS_RPC_PORT="8545"
GESS_RPC_URL="http://${GESS_IP}:${GESS_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gess and
# swarm nodes
start_bootnode() {
...
# start_gess_node starts a gess node with --datadir pointing at <base-dir>/gess
# and a single, unlocked account with password "gess"
start_gess_node() {
  local dir="${base_dir}/gess"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gess account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GESS_IP}"
    --rpcport   "${GESS_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gess" "${GESS_IP}" "$(which gess)" ${args[@]}
}
...
create_account() {
  ...
  gess --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
