...
import (
	...
	gessmetrics "github.com/orangeAndSuns/essentia/metrics"
...
func Setup(ctx *cli.Context) {
	if gessmetrics.Enabled {
		...
		go gessmetrics.CollectProcessMetrics(2 * time.Second)

		if enableExport {
			log.Info("Enabling swarm metrics export to InfluxDB")
			go influxdb.InfluxDBWithTags(gessmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
