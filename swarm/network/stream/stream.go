...
import (
	...
	"github.com/orangeAndSuns/essentia/swarm/network/stream/intervals"
	"github.com/orangeAndSuns/essentia/swarm/pot"
	"github.com/orangeAndSuns/essentia/swarm/spancontext"
...
type Registry struct {
	...
	peers          map[discover.ESSNodeID]*Peer
	delivery       *Delivery
...
func (r *Registry) getPeer(peerId discover.ESSNodeID) *Peer {
	r.peersMu.RLock()
...
func (r *Registry) updateSyncing() {
	...
	subs := make(map[discover.ESSNodeID]map[Stream]struct{})
	r.peersMu.RLock()
...
func (api *API) SubscribeStream(peerId discover.ESSNodeID, s Stream, history *Range, priority uint8) error {
	return api.streamer.Subscribe(peerId, s, history, priority)
}

func (api *API) UnsubscribeStream(peerId discover.ESSNodeID, s Stream) error {
