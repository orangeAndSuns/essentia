...
import (
	...
	"github.com/orangeAndSuns/essentia/contracts/chequebook"
	"github.com/orangeAndSuns/essentia/contracts/ens"
	...
	"github.com/orangeAndSuns/essentia/p2p/protocols"
	"github.com/orangeAndSuns/essentia/params"
	"github.com/orangeAndSuns/essentia/rpc"
	"github.com/orangeAndSuns/essentia/swarm/api"
	httpapi "github.com/orangeAndSuns/essentia/swarm/api/http"
	"github.com/orangeAndSuns/essentia/swarm/fuse"
	"github.com/orangeAndSuns/essentia/swarm/log"
	"github.com/orangeAndSuns/essentia/swarm/network"
	"github.com/orangeAndSuns/essentia/swarm/network/stream"
	"github.com/orangeAndSuns/essentia/swarm/pss"
	"github.com/orangeAndSuns/essentia/swarm/state"
	"github.com/orangeAndSuns/essentia/swarm/storage"
	"github.com/orangeAndSuns/essentia/swarm/storage/mock"
	"github.com/orangeAndSuns/essentia/swarm/storage/mru"
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = essclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	essClient := essclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, essClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := essclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
