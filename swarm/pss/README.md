...
The code tutorial [p2p programming in essentia](https://github.com/nolash/essentia-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `essentia` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `essentia` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
