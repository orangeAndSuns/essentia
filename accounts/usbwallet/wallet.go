...
type wallet struct {
	...
	deriveChain    essentia.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain essentia.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
