...
import (
	...
	"github.com/orangeAndSuns/essentia/accounts/usbwallet/internal/trezor"
...
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EssentiaAddress)
	if _, err := w.trezorExchange(&trezor.EssentiaGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.EssentiaTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EssentiaTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
