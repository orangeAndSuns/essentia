...
type Database struct {
	diskdb essdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb essdb.Database) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > essdb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch essdb.Batch) error {
