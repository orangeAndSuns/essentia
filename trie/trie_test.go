...
// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(essdb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(essdb.NewMemDatabase()))
	if trie != nil {
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(essdb.NewMemDatabase())
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*essdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := essdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
