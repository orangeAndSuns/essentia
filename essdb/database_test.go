...
func newTestLDB() (*essdb.LDBDatabase, func()) {
	dirname, err := ioutil.TempDir(os.TempDir(), "essdb_test_")
	if err != nil {
		panic("failed to create test file: " + err.Error())
	}
	db, err := essdb.NewLDBDatabase(dirname, 0, 0)
	if err != nil {
...
func TestMemoryDB_PutGet(t *testing.T) {
	testPutGet(essdb.NewMemDatabase(), t)
}

func testPutGet(db essdb.Database, t *testing.T) {
	t.Parallel()
...
func TestMemoryDB_ParallelPutGet(t *testing.T) {
	testParallelPutGet(essdb.NewMemDatabase(), t)
}

func testParallelPutGet(db essdb.Database, t *testing.T) {
	const n = 8
