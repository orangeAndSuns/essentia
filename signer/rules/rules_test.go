...
import (
	...
	"github.com/orangeAndSuns/essentia/signer/core"
	"github.com/orangeAndSuns/essentia/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx essapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx essapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(essapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := essapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx essapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
